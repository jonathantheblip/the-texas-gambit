...
- **Branch reconciliation (pre-deploy)** ⚠️→✅ — `origin/main` had diverged with 2 commits (improved **shared Texas-silhouette PWA icon** + cache-bust) touching old-app files my branch had retired. Adopted the improved icon PNGs into `public/` (helen+jon 180/512); will merge `origin/main` with `-s ours` so its history is preserved (not lost) while `main` becomes the new Vite app. Nothing of value discarded (old root app.jsx/helen.html/jon.html/per-user manifests don't exist in the new app).
