...
- **DEPLOYED — LIVE** ✅ (2026-06-29)
  - Push was first **denied (403)**: the session credential `jonathan-crescent` had READ-only access, and switching to the owner account was guardrail-blocked (not worked around). **Jon granted `jonathan-crescent` Write on the repo** → `git push origin main` succeeded (`840c496..e269646`). `render-forward-3d` pushed too (backup).
  - **CI** passed (validate/locks ✓); **Deploy to GitHub Pages** workflow completed **success**. Live URL **https://jonathantheblip.github.io/the-texas-gambit/** returns **200** and serves the new Vite app (bundle `index-CDBK8s3o.js` — byte-identical to the production build verified locally). **It is now Helen's live app.**
  - **Remaining post-deploy check (needs a real device, can't be done in-sandbox):** confirm cross-device **sync** — leave a note on one device, see it on another. The code is additive + degrades to local; worth a quick confirm.
...
- **Post-deploy 404 fix** ✅ — Helen's home-screen icon (installed from the OLD app, standalone mode) opened an old entry path (`helen.html` / `jon.html` / `Hill Country Estate.html`) that the new Vite app doesn't have → GitHub 404. The new app itself was fine (root + `/index.html` = 200). Added redirect stubs at those three paths (`public/*.html` → `location.replace('./')`) so her existing icon forwards into the new app with no action from her. Verified locally: loading `/the-texas-gambit/helen.html` lands on the new gallery.
- **Deployed & live** at https://jonathantheblip.github.io/the-texas-gambit/ (2026-06-29). Remaining checks (real device): Helen's icon now loads; cross-device sync. Optional: Helen can re-add to home screen from the current URL for the new icon + no redirect hop.
