...
2. **Walk-between navigation.** Code now ships an **adjacency engine** (`src/data/adjacency.js` → `neighborsOf(id)`) and a functional *"Walk to an adjoining space"* strip in the room view — from any room you can step to its wall-adjacent / stacked-above-or-below neighbors (render thumb + compass/up-down direction). It works on mobile today. **Your job is to make it *feel* like walking:** a directional transition between rooms (slide/settle toward the heading), a persistent thumb-reachable walk control, a "you are here" sense (a small compass or compound minimap — Code can derive the footprints), and the render as the moment of arrival. Extend the same drift into the 3D massing (camera move between spaces) when that view is in play. (Refinements Code can add on request: routing vertical moves through actual stairs, and a natural entry point so the walk starts at the Front Porch.)
...
5. **Colored-pencil renders *in* the 3D (Jon's standing aspiration).** Evolve the massing from gray boxes toward the actual illustrations living in 3D — e.g. a "diorama" of the renders standing in their places, or the massing blocks skinned with their renders. Honest constraint: the renders are single flat illustrations, not 360° panoramas, so a literal "stand inside the painting and look around" needs *new* 360 art from the render pipeline (Prompt Bible side), not just a Code change. Code can prototype the diorama/skinned-massing versions from the existing flat assets today.
