...
- **Wiring (Code):** merge the batch into `pins.json`, spot-check one room in the preview, deploy. (All 52 render-bearing rooms pinned as of this pass; new rooms/renders would re-open the list.)
...
## Open / next
1. **Render pins — content complete (52/52).** Design's treatment is integrated and every render-bearing room is pinned. The merge→spot-check→deploy loop stays available for any *new* rooms/renders. On-device feel of the new breathing pins + look-closer is a nice phone confirm for Jon.
